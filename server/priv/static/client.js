...
function float_to_string(float2) {
  const string4 = float2.toString().replace("+", "");
...
class Snippet extends CustomType {
  constructor(id, author_id, author_name, title2, content, version, expires_at, updated_at, created_at) {
    super();
    this.id = id;
...
function snippet_decoder() {
  return field("id", int2, (id) => {
    return field("author_id", int2, (author_id) => {
      return field("author_name", string3, (author_name) => {
        return field("title", string3, (title2) => {
          return field("content", string3, (content) => {
              ...
              return field("expires_at", int2, (expires_at) => {
                return field("updated_at", int2, (updated_at) => {
                  return field("created_at", int2, (created_at) => {
                    return success(new Snippet(id, author_id, author_name, title2, content, version, expires_at, updated_at, created_at));
                  });
                });
              });
            });
...
function postMessage(window2, message2, targetOrigin) {
  window2.postMessage(message2, targetOrigin);
}
function onMessage(worker2, callback) {
  worker2.addEventListener("message", function(message2) {
    callback(message2);
  });
}
...
// build/dev/javascript/client/client/route.mjs
class Home extends CustomType {
}
class ShowSnippet extends CustomType {
  constructor(snippet_id) {
    super();
...
function init(snippets) {
  let model = new Model(snippets, new None, new Home, false, new None);
  return [model, none()];
}
function create_snippet_to_json(create_snippet) {
  ...
  return object2(toList([["title", string4(title2)], ["content", string4(content)]]));
}
...
function update2(model, msg) {
  if (msg instanceof ServerSavedSnippet) {
    ...
    if (snippet instanceof Ok2) {
      ...
      return [
        new Model(model.snippets, new Some(snippet$1), (() => {
          throw makeError("todo", FILEPATH, "client", 86, "update", "`todo` expression evaluated. This code has not yet been implemented.", {});
        })(), model.saving, model.error),
        none()
      ];
    } else {
      return [
        new Model(model.snippets, new None, (() => {
          throw makeError("todo", FILEPATH, "client", 91, "update", "`todo` expression evaluated. This code has not yet been implemented.", {});
        })(), model.saving, model.error),
        none()
      ];
    }
...
function view(model) {
  ...
  } else if ($ instanceof ShowSnippet) {
    let id = $.snippet_id;
    return div(toList([]), toList([
      h1(toList([class$("text-blue-500 text-2xl")]), toList([text3("Snippet")])),
      p(toList([]), toList([text3(to_string(id))]))
    ]));
  } else {
    return none2();
  }
...
function main() {
  ...
  if (!($ instanceof Ok2)) {
    throw makeError("let_assert", FILEPATH, "client", 29, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 808, end: 867, pattern_start: 819, pattern_end: 824 });
  }
  return;
}
